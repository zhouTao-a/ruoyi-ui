    ...
    <el-card shadow="never">
      <template #header>
        <el-row :gutter="10" class="mb8">
          ...
          <el-col :span="1.5">
            <el-button type="success" plain icon="Edit" :disabled="single" @click="handleUpdate()" v-hasPermi="['msg:msgMatterGroup:edit']">修改</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="danger" plain icon="Delete" :disabled="multiple" @click="handleDelete()" v-hasPermi="['msg:msgMatterGroup:remove']">删除</el-button>
          </el-col>
          <el-col :span="1.5">
...
const handleDelete = async (row?: MsgMatterGroupVO) => {
  ...
  await proxy?.$modal.confirm('是否确认删除事件组编号为"' + _ids + '"的数据项？').finally(() => loading.value = false);
...
/** 导出按钮操作 */
const handleExport = () => {
  proxy?.download('msg/msgMatterGroup/export', {
    ...queryParams.value
